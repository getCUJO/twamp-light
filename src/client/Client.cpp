...
make_qed_observation(ObservationPoints observation_point,
		     uint64_t epoch_nanoseconds,
		     uint32_t packet_id,
		     uint16_t payload_len)
{
	struct qed_observation *obs = (struct qed_observation *)malloc(
		sizeof(struct qed_observation));
...
void Client::runSenderThread()
{
	...
	std::exponential_distribution<> d(
		1.0 / (args.mean_inter_packet_delay *
		       1000)); // Lambda is 1.0/mean (in microseconds)
	while (args.num_samples == 0 || index < args.num_samples ||
	       (args.runtime != 0 &&
		time(NULL) - this->start_time < args.runtime)) {
		...
		if (args.constant_inter_packet_delay) {
			...
			delay = std::max((double)std::min((double)d(gen),
							  10000000.0),
					 0.0);
		}
		usleep(delay);
...
void Client::runReceiverThread()
{
	...
	while ((args.num_samples == 0 || this->sending_completed == 0 ||
		(this->received_packets < this->sent_packets &&
		 time(NULL) - this->sending_completed < args.timeout))) {
...
void Client::process_observation(struct qed_observation *obs)
{
	...
	case ObservationPoints::SERVER_RECEIVE:
		entry->server_receive_epoch_nanoseconds =
			obs->epoch_nanoseconds;
		break;
	...
	case ObservationPoints::CLIENT_RECEIVE:
		entry->client_receive_epoch_nanoseconds =
			obs->epoch_nanoseconds;
		break;
	default:
		break;
...
void Client::check_if_oldest_packet_should_be_processed()
{
	...
	if (oldest_raw_data != NULL &&
	    (oldest_entry_is_complete > 0 ||
	     (now_nanoseconds - oldest_raw_data->added_at_epoch_nanoseconds) >
		...
		if (raw_data_list->num_entries > 0) {
			raw_data_list->oldest_entry =
				raw_data_list->oldest_entry->next;
			if (raw_data_list->oldest_entry != NULL) {
...
void Client::runCollatorThread()
{
	...
	while (collator_finished == 0) {
		...
		if (sem_trywait(&observation_semaphore) == 0) {
			...
			memcpy(tmp_obs_ptr,
			       observation_list->first->observation,
			       sizeof(struct qed_observation));
			struct observation_list_entry *processed_entry =
				observation_list->first;
...
int64_t calculate_correction(RawData **first_entry, RawData **last_entry)
{
	...
	while (current_entry <= last_entry) {
		...
		int64_t fwd = entry->server_receive_epoch_nanoseconds -
			      entry->client_send_epoch_nanoseconds;
		if (fwd < min_fwd) {
			min_fwd = fwd;
		}
		int64_t bwd = entry->client_receive_epoch_nanoseconds -
			      entry->server_send_epoch_nanoseconds;
		if (bwd < min_bwd) {
			min_bwd = bwd;
		}
		current_entry++;
...
void Client::aggregateRawData(RawData *oldest_raw_data)
{
	...
	if (oldest_raw_data->client_send_epoch_nanoseconds > 0 &&
		...
		client_server_delay = nanosecondsToTimespec(
			oldest_raw_data->server_receive_epoch_nanoseconds -
			oldest_raw_data->client_send_epoch_nanoseconds);
...
bool Client::awaitAndHandleResponse()
{
	...
	char buffer[sizeof(
		ReflectorPacket)]; // We should only be receiving ReflectorPackets
	char control[2048];
	...
	struct msghdr incoming_msg = make_msghdr(
		&iov, 1, &src_addr, sizeof(src_addr), control, sizeof(control));
	...
	if (count == -1) {
		...
		handleReflectorPacket(
			rec, incoming_msg, count, incoming_timestamp_ptr);
	}
	return true;
}
			 ...
			 TimeSynchronizer *timeSynchronizer)
{
	...
	if (sync_time) {
		uint32_t server_timestamp =
			ntohl(reflectorPacket->server_time_data.integer);
		uint32_t server_delta =
			ntohl(reflectorPacket->server_time_data.fractional);
		uint32_t client_timestamp =
			ntohl(reflectorPacket->client_time_data.integer);

		int64_t server_client_delay =
			timeSynchronizer->OnAuthenticatedDatagramTimestamp(
				server_timestamp, client_receive_time);
		timeSynchronizer->OnPeerMinDeltaTS24(server_delta);

		auto a = timeSynchronizer->To64BitUSec(
			client_receive_time,
			timeSynchronizer->ToRemoteTime23(
				timeSynchronizer->To64BitUSec(
					client_receive_time,
					client_timestamp)));
		auto b = timeSynchronizer->To64BitUSec(client_receive_time,
						       server_timestamp);
		int64_t client_server_delay = (int64_t)(b - a);

		timeData.client_send_time = timeSynchronizer->To64BitUSec(
			client_receive_time, client_timestamp);
		timeData.server_receive_time = timeSynchronizer->To64BitUSec(
			client_receive_time, server_timestamp);
		timeData.server_send_time = timeSynchronizer->To64BitUSec(
			client_receive_time,
			ntohl(reflectorPacket->send_time_data.integer));

		timeData.internal_delay =
			(int64_t)(timeData.server_send_time -
				  timeData.server_receive_time);
		timeData.rtt = (int64_t)(client_receive_time -
					 timeData.client_send_time);
		timeData.client_server_delay = client_server_delay;
		timeData.server_client_delay = server_client_delay;
	} else {
		auto client_timestamp =
			ntohts(reflectorPacket->client_time_data);
		...
		timeData.server_receive_time =
			timestamp_to_nsec(&server_timestamp);
		timeData.server_send_time = timestamp_to_nsec(&send_timestamp);

		timeData.internal_delay = timeData.server_send_time -
					  timeData.server_receive_time;
		timeData.client_server_delay = timeData.server_receive_time -
					       timeData.client_send_time;
		timeData.server_client_delay =
			client_receive_time - timeData.server_send_time;
...
void Client::enqueue_observation(struct qed_observation *obs)
{
	...
	struct observation_list_entry *entry =
		(struct observation_list_entry *)malloc(
			sizeof(struct observation_list_entry));
				   ...
				   timespec *incoming_timestamp)
{
	...
	if (incoming_timestamp->tv_sec == 0 &&
		...
		incoming_timestamp_nanoseconds =
			(uint64_t)incoming_timestamp->tv_sec * 1000000000 +
	...
	if (this->collator_started) {
		struct qed_observation *obs1 =
			make_qed_observation(ObservationPoints::CLIENT_RECEIVE,
					     incoming_timestamp_nanoseconds,
					     packet_id,
					     payload_len);
		struct qed_observation *obs2 = make_qed_observation(
			ObservationPoints::SERVER_RECEIVE,
			timestamp_to_nsec(&server_receive_time),
			packet_id,
			payload_len);
		struct qed_observation *obs3 = make_qed_observation(
				  ...
				  struct sqa_stats *stats)
{
	...
	uint16_t local_port = atoi(args.local_port.c_str());
	TimeData timeData = computeTimeData(args.sync_time,
					    client_receive_time,
					    reflectorPacket,
					    timeSynchronizer);
...
{
	char sync = 'N';
	uint64_t estimated_rtt = data.client_server_delay +
				 data.server_client_delay + data.internal_delay;
	if (isWithinEpsilon((double)data.rtt_delay * 1e-6,
			    (double)estimated_rtt * 1e-6,
			    0.01)) {
		sync = 'Y';
	}
	...
	std::cout
		...
		<< (double)data.server_client_delay * 1e-6 << args.sep
		<< data.payload_length;
	if (args.print_lost_packets) {
...
void Client::JsonLog(std::string json_output_file)
{
	...
	time_t first_sent_seconds =
		Client::first_packet_sent_epoch_nanoseconds / 1e9;
	int microseconds =
		Client::first_packet_sent_epoch_nanoseconds % 1000000000;
	auto now_as_tm_date = std::gmtime(&first_sent_seconds);
	char first_packet_sent_date[80];
	strftime(first_packet_sent_date,
		 sizeof(first_packet_sent_date),
		 "%Y-%m-%dT%H:%M:%S",
		 now_as_tm_date);
	// Add the microseconds back in:
	char first_packet_sent_date_with_microseconds[91];
	sprintf(first_packet_sent_date_with_microseconds,
		"%s.%06dZ",
		first_packet_sent_date,
		microseconds);
...
{
	struct observation_list *res = (struct observation_list *)malloc(
		sizeof(struct observation_list));
	res->first = NULL;
	res->last = NULL;
	return res;
}
