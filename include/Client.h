...
class RawData {
	...
	RawData(uint64_t added_at_epoch_nanoseconds,
		...
		: added_at_epoch_nanoseconds(added_at_epoch_nanoseconds)
		, packet_id(packet_id)
	...
	void
	setClientSendEpochNanoseconds(uint64_t client_send_epoch_nanoseconds)
	{
		this->client_send_epoch_nanoseconds =
			client_send_epoch_nanoseconds;
	}
	void setServerReceiveEpochNanoseconds(
		uint64_t server_receive_epoch_nanoseconds)
	{
		this->server_receive_epoch_nanoseconds =
			server_receive_epoch_nanoseconds;
	}
	void
	setServerSendEpochNanoseconds(uint64_t server_send_epoch_nanoseconds)
	{
		this->server_send_epoch_nanoseconds =
			server_send_epoch_nanoseconds;
	}
	void setClientReceiveEpochNanoseconds(
		uint64_t client_receive_epoch_nanoseconds)
	{
		this->client_receive_epoch_nanoseconds =
			client_receive_epoch_nanoseconds;
	}
};
...
class Client {
	...
	void print_lost_packet(uint32_t packet_id,
	...
	void printSummaryLine(const std::string &label, Func func);
