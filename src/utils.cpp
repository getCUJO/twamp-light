...
void timeval_to_timestamp(const struct timeval *tv, Timestamp *ts)
{
	...
	ts->integer = tv->tv_sec + NTP_EPOCH_OFFSET;
	ts->fractional =
		(uint32_t)((double)tv->tv_usec *
			   ((double)(1ULL << 32) / MICROSECONDS_IN_SECOND));
}
...
void timespec_to_timestamp(const struct timespec *tv, Timestamp *ts)
{
	...
	ts->integer = tv->tv_sec + NTP_EPOCH_OFFSET;
	ts->fractional =
		(uint32_t)((double)tv->tv_nsec *
			   ((double)(1ULL << 32) / NANOSECONDS_IN_SECOND));
}
...
void timestamp_to_timeval(const Timestamp *ts, struct timeval *tv)
{
	...
	tv->tv_usec =
		(__suseconds_t)((double)ts_host_ord.fractional *
				MICROSECONDS_IN_SECOND / (double)(1ULL << 32));
}
...
void timestamp_to_timespec(const Timestamp *ts, struct timespec *tv)
{
	...
	/* NTP to Unix time */
	tv->tv_sec = ts_host_ord.integer - NTP_EPOCH_OFFSET;
	tv->tv_nsec = (__syscall_slong_t)((double)ts_host_ord.fractional *
...
auto timestamp_to_usec(const Timestamp *ts) -> uint64_t
{
	...
	return (uint64_t)tv.tv_sec * MICROSEONDS_IN_SECOND_INT +
	       (uint64_t)tv.tv_usec;
}
...
auto timestamp_to_nsec(const Timestamp *ts) -> uint64_t
{
	...
	return (uint64_t)tv.tv_sec * NANOSECONDS_IN_SECOND_INT +
	       (uint64_t)tv.tv_nsec;
}

auto nanosecondsToTimespec(uint64_t delay_epoch_nanoseconds)
	-> struct timespec {
	struct timespec ts {};
	ts.tv_sec = static_cast<__time_t>(delay_epoch_nanoseconds /
					  NANOSECONDS_IN_SECOND_INT);
	ts.tv_nsec = static_cast<__syscall_slong_t>(delay_epoch_nanoseconds %
						    NANOSECONDS_IN_SECOND_INT);
	return ts;
}
...
auto get_ip_header(msghdr hdr) -> IPHeader
{
	...
	for (c_msg = CMSG_FIRSTHDR(&hdr); c_msg != nullptr;
		...
		case IPPROTO_IP:
			...
			default:
				std::cerr
					<< "\tWarning, unexpected data of level "
					<< c_msg->cmsg_level << " and type "
...
{
	struct cmsghdr *cm{};
	for (cm = CMSG_FIRSTHDR(&incoming_msg); cm != nullptr;
	     cm = CMSG_NXTHDR(&incoming_msg, cm)) {
		if (cm->cmsg_level != SOL_SOCKET)
			continue;
		switch (cm->cmsg_type) {
		case SO_TIMESTAMPNS:
		case SO_TIMESTAMPING:
			memcpy(incoming_timestamp,
			       static_cast<void *>(CMSG_DATA(cm)),
		...
		case SO_TIMESTAMP: {
			...
			incoming_timestamp->tv_nsec =
				tv.tv_usec *
				NANOSECONDS_IN_MICROSECOND; // Convert microseconds to nanoseconds
			break;
		}
		default:
...
void set_socket_options(int socket, uint8_t ip_ttl, uint8_t timeout_secs)
{
	...
	int flags = 1;
	if (setsockopt(
		    socket, SOL_SOCKET, SO_TIMESTAMPNS, &flags, sizeof(flags)) <
	    0)
...
{
	return (std::abs(a - b) <=
		(std::max(std::abs(a), std::abs(b)) * percentEpsilon));
}
