...
auto parse_args(int argc, char **argv) -> Args
{
	...
	app.add_option(
		   "-t, --timeout",
		   args.timeout,
		   "How long (in seconds) to wait for response before aborting.")
		->default_str(std::to_string(args.timeout));
	app.add_option(
		"-s, --seed", args.seed, "Seed for the RNG. 0 means random.");
	app.add_flag("--print-digest{true}",
		     args.print_digest,
		     "Prints a statistical summary at the end.");
	app.add_option("-j, --json-output",
		       args.json_output_file,
		       "Filename to dump json output to");
	app.add_flag(
		"--print-lost-packets",
		args.print_lost_packets,
		"Prints sent and lost packet counters, legacy format only");
	app.add_option("--print-RTT-only",
		       args.print_RTT_only,
		       "Prints only the RTT values.");
	app.add_option(
		   "--print-format",
		   args.print_format,
	...
	app.add_option(
		"--sep", args.sep, "The separator to use in the output.");
	...
	if (print_version) {
		std::cout << "Twamp Light Version "
			  << std::string(TWAMP_VERSION_TXT) << std::endl;
		std::cout << "Commit ID " << std::string(TWAMP_GIT_COMMIT_ID);
		std::cout << ", git describe: "
			  << std::string(TWAMP_GIT_DESCRIBE);
		std::cout << ",  submodules: qoo-c ("
